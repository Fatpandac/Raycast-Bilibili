...
export const API = {
  ...
  popularSeriesVideos: (number: number) =>
    `${BASE_API_URL}/x/web-interface/popular/series/one?number=${number}`,
  videoInfo: (id: string) => `${BASE_API_URL}/x/web-interface/view?aid=${id}`,
  playUrl: (bvid: string, cid: string) => `${BASE_API_URL}/x/player/playurl?bvid=${bvid}&cid=${cid}&qn=112&fnval=0&fnver=0&fourk=1`,
};
